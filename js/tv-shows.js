...
function getTVs() {
    axios
        .get(
            "https://api.themoviedb.org/3/tv/popular?&language=en-US&region=US&api_key=4f3fdbd8c5943719506e53611dd7be34&language=en-US&page=1"
        )
        .then((response) => {
            var trending = "<ul>";
            let word = "";
            ...
            for (i = 0; i <= 7; i++) {
                ...
                trending += `<a onclick="TVSelected(${
                    response.data.results[i].id
                })" href='#'><li id="charts-item"><img class="charts-poster" src='${
            ...
            trending += "</ul>";
            trending +=
                '<a href="TV_trending.html" class="charts-more">More trending ></a>';
    ...
    axios
        .get(
            "https://api.themoviedb.org/3/tv/on_the_air?page=1&language=en-US&region=US&api_key=4f3fdbd8c5943719506e53611dd7be34"
        )
        .then((response) => {
            var now_playing = `<ul>`;
            let word = "";
            ...
            for (i = 0; i <= 7; i++) {
                ...
                now_playing += `<a onclick="TVSelected(${
                    response.data.results[i].id
                })" href='#'><li id="charts-item"><img class="charts-poster" src='${
                    response.data.results[i].poster_path != null
                        ? "https://image.tmdb.org/t/p/w200" +
            ...
            now_playing += `</ul>`;
            now_playing +=
                '<a href="TV_now_playing.html" class="charts-more">More playing ></a>';
    ...
    axios
        .get(
            "https://api.themoviedb.org/3/tv/airing_today?page=1&language=en-US&region=US&api_key=4f3fdbd8c5943719506e53611dd7be34"
        )
        .then((response) => {
            var upcoming = `<ul>`;
            let word = "";
            ...
            upcoming += `</ul>`;
            upcoming +=
                '<a href="TV_upcoming.html" class="charts-more">More upcoming ></a>';
        ...
        .then((response) => {
            ...
            for (let i = 0; i <= tv.seasons.length - 1; i++) {
                if (tv.seasons[i].name == x) {
                    ...
                    document.getElementById("air-date").innerHTML =
                        "Air date: " + tv.seasons[i].air_date;
    ...
    axios
        ...
        .then((response) => {
            ...
            for (i = 0; i <= response.data.results.length - 1; i++) {
                ...
                list += `<div class="charts-list-score"><i id="charts-list-star" class="fas fa-star"></i><span class="vote-average">${
                    response.data.results[i].vote_average
                }</span><span class="vote-count">(${
                    response.data.results[i].vote_count + " " + word
                })</span></div></li></a>`;
            ...
            if (x == 1 && response.data.total_pages > 1) {
                list += `<a href="#" onclick="getTrendingTVCharts(${
                    x + 1
                })">Next page</a>`;
...
function getPlayingTVCharts(x) {
    ...
    axios
        .get(
            "https://api.themoviedb.org/3/tv/on_the_air?page=" +
                x +
                "&language=en-US&region=US&api_key=4f3fdbd8c5943719506e53611dd7be34"
        )
        .then((response) => {
            ...
            if (x == 1 && response.data.total_pages > 1) {
                list += `<a href="#" onclick="getPlayingTVCharts(${
                    x + 1
                })">Next page</a>`;
            } else if (x > 1 && response.data.total_pages > 1) {
                list += `<a href="#" onclick="getPlayingTVCharts(${
...
function getUpcomingTVCharts(x) {
    ...
    axios
        ...
        .then((response) => {
            ...
            if (x == 1 && response.data.total_pages > 1) {
                list += `<a href="#" onclick="getUpcomingTVCharts(${
                    x + 1
                })">Next page</a>`;
            } else if (x > 1 && response.data.total_pages > 1) {
                list += `<a href="#" onclick="getUpcomingTVCharts(${
